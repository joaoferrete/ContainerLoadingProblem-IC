...
typedef struct Individuo {
    ...
    Individuo(vector <bool> cromossomo) {
        this->cromossomo = cromossomo;
        fitness = -1;
    }

    Individuo & operator=(const Individuo & individuo) {
        this->cromossomo = individuo.cromossomo;
        this->fitness = individuo.fitness;
        return *this;
    }

    Individuo cruzar (Individuo individuo) {
        vector <bool> filho;
        filho.push_back(0);
        for(long i = 1; i < cromossomo.size(); i++) {
            int corte = rand() % 100;
            //cout <<"CORTE: " << corte << endl;
            if(corte < 45) filho.push_back(cromossomo[i]);
            else if(corte < 90) filho.push_back(individuo.cromossomo[i]);
            else filho.push_back(rand() % 2);
        }

        return Individuo(filho);
    }

    bool operator<(const Individuo & individuo) const {
        return this->fitness < individuo.fitness;
    }
} Individuo;
...
vector <Individuo> criarPopulacao(){
    vector <Individuo> populacao;
    for(int i = 0; i < TAM_POPULACAO; i++){
        vector <bool> cromossomo(tamCx.size());
        for(int j = 1; j < cromossomo.size() ; j++){
            cromossomo[j] = rand() % 2;
        }
        populacao.push_back(Individuo(cromossomo));
    }
    return populacao;
}

double Objetivo(vector <bool> s){
    long long punicao = 1;
    for(int i = 1; i < s.size(); i++){
        if(!s[i] && tamCx[i].priority == 2)
            punicao += 100;
    }
    ...
    try{
        ...
        for(long i = 1; i <= numCx; i++){
            volUtilizado += tamCx[i].x*tamCx[i].y*tamCx[i].z*s[i];
        }
        ...
        cb = (rand() % 255)/255.0;
        int j=0;
        if(cplex.getObjValue() / punicao > MELHOR_INDIVIDUO){
            caixas.clear();
            MELHOR_INDIVIDUO = cplex.getObjValue() / punicao;
            ...
            for(int i = 1; i <= numCx; i++){
                    ...
                    if(s[i]){   // Se a caixa está empilhada
        ...
        return cplex.getObjValue() / punicao;
...
void calculaFitness(vector<Individuo>& populacao){
    for(int i = 0; i < populacao.size(); i++){
        cout << "Individuo: ";
        for(int j = 1; j < populacao[i].cromossomo.size(); j++){
            cout << populacao[i].cromossomo[j];
        }
        cout << "   Fitness: ";
        if(populacao[i].fitness == -1)populacao[i].fitness = Objetivo(populacao[i].cromossomo);
        cout << populacao[i].fitness << endl;
    }
}
...
int main(int argc, char **argv){
    ...
    vector <Individuo> populacao = criarPopulacao();
    for(int k = 0; k < MAX_GERACOES; k++){
        cout << "Geração: " << k << endl;
        ...
        int apai1 = 0, apai2 = 0, pai1 = 0, pai2 = 0;
        vector <Individuo> novaPopulacao;
        for(int i = 0; i < TAM_POPULACAO; i++){
            if(i <= populacao.size()*0.15){
                novaPopulacao.push_back(populacao[i]);
            }
            else{
                while(apai1 == pai1 && apai2 == pai2){
                    pai1 = rand() % (TAM_POPULACAO/2);
                    pai2 = rand() % (TAM_POPULACAO/2);
                }
                while(pai1 == pai2) pai2 = rand() % (TAM_POPULACAO/2);
                
                //cout<<"Pai 1: "<<pai1<<"   Pai 2: "<<pai2<<endl;
                Individuo filho = populacao[pai1].cruzar(populacao[pai2]);
    ...
    for(int i = 1; i < populacao[0].cromossomo.size(); i++){
        cout << populacao[0].cromossomo[i];
    }
