...
int main(int argc, char **argv){
    ...
    try{
        ...
        for(int i = 1; i <= numCx; i++){
            cout << "Caixa " << i << ": ";
            cout << "Está na posição (" << cplex.getValue(pos[i][0]) << ", " << cplex.getValue(pos[i][1]) << ", " << cplex.getValue(pos[i][2]) << ")";
